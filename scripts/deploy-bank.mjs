// Deploy AscendBank to Paseo (raw provider; Paseo receipts are flaky).
//   node scripts/deploy-bank.mjs
...
async function main() {
  ...
  const art = JSON.parse(readFileSync("artifacts/AscendBank.json", "utf8"));
  ...
  for (let i = 0; i < 120; i++) {
    ...
    if ((await p.getCode(addr).catch(() => "0x")) !== "0x") {
      console.log("AscendBank deployed →", addr);
      writeFileSync("deployed-bank.json", JSON.stringify({ network: "paseo", AscendBank: addr, deployedAt: new Date().toISOString() }, null, 2));
      return;
    }
  }
