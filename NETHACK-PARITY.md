...
- **Phase 6 — The character sheet.** Six attributes (flavored), XP/levels ("epochs"), the d20 to-hit/AC formula, weight + encumbrance, and a first set of **archetypes** (start with 3–4: Validator, Nominator, Cypherpunk, Builder) with distinct starting stats/kits. *Foundational — everything downstream leans on it.*
- **Phase 7 — Items, deep.** Multi-slot armor (7 slots) + erosion/proofing; corpses + eating (nutrition/intrinsics/afflictions) + the timeout engine (stoning/slime/illness countdowns); tools class (pick-axe, unicorn-horn, magic marker, leash, crystal ball, lamp); containers (multisig vault, chest); light/darkness/blindness; detection family; amulets (incl. life-saving, reflection); the ray/`zap` engine + the full wand list.
