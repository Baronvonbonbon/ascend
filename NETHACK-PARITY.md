...
- **Phase 10 — The bestiary.** 🟡 *10a shipped:* four new foes + two new abilities — a **finality dragon** (`D`) with a **breath ray** (a damaging line-attack that bounces, hitting anything in its path), a **sudo conjurer** (`&`) that **summons** depth-appropriate reinforcements, a fast **MEV bot** (`b`), and a hard-hitting **slashing daemon** (`S`). *Still to do:* dozens more across classes; monster item-use (`muse` — drink/zap/throw) + spellcasting + breeding; demons; mercenary/watch `@`-folk; riding/steeds; priests/guards.
