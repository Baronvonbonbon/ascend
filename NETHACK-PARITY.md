...
- **Phase 11 — The verb set.** 🟡 *11a shipped:* **Quiver + fire** — `Q` readies a missile (weapon or potion) in a quiver slot, then `f` fires it down a direction without re-rummaging the pack; the slot **auto-refills** from the next identical missile when one's spent (and `f` with an empty quiver auto-loads the first throwable), so you can plink a closing foe shot after shot. The readied piece shows `(at the ready)` in the pack. *Still to do:* `#dip`, `#loot`, `#force`, `#kick`, door open/close/lock, `#chat`, `#ride`, `#twoweapon`, `#enhance` (weapon skills), `;`/`/`/`^` info, `#name`, discoveries screen.
