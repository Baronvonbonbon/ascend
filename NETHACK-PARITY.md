...
- **Phase 7 — Items, deep.** 🟡 *7a shipped:* **corpses** drop on kill (eat in place with `e`) → nutrition + **intrinsics** (poison resistance, intrinsic speed) or danger; the **affliction/timeout engine** — eating a **freezer** (cockatrice) corpse starts **stoning** (`Ston`, deadly countdown), rotten corpses cause **illness** (`Ill`), both cured by prayer or a cleanse; poison resistance now negates poison; intrinsics shown on the `@` sheet. *Still to do (7b+):* multi-slot armor (7 slots) + erosion/proofing; tools (pick-axe, unicorn-horn, magic marker, leash, crystal ball, lamp); containers (multisig vault, chest); light/darkness/blindness; detection family; amulets (life-saving, reflection); the ray/`zap` engine + full wand list.
