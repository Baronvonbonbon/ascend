...
- **Phase 9 — The dungeon, branched.** 🟡 *9a shipped:* **faucets** (`{`, testnet fountains — `q` to quaff for a random boon/bane: heal, a faucet bot, poison, a HODL stone, or it runs dry) and **thrones** (`\`, the Sudo Throne — `s` to sit for full-heal+Fortune, a curse, teleport, pack identify, a level, a guardian, or it crumbles), placed per level. *Still to do:* a level-definition layer + branch graph; **the Mempool Mines** and **the Consensus Vault (Sokoban)**; sinks/temples/vaults; lockable doors + chests; expanded trap table; the shop **bill ledger**.
