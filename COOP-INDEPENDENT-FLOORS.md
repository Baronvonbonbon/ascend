...
- [x] **Independent floors** — players can be on different floors (`dungeon:N`,
 a dungeon, a branch, a plane) at the same time, both **fully simulated**
...
`wilds:5` (dungeon), `mines:2` (branch floor), `plane:3`, `quest`.
...
- [x] **Tag mid-play spawns** so they never carry a stale default key. Done at the
 ...
 breeders, faucet/throne bots, the Warden resurrection, minibosses, branch
...
- [x] Edge cases: regroup → shared slot, monsters scheduled once (Set dedupe);
 ...
 Amulet-bearer's floor; `enterPlane` now sets `activeKey = plane:n` and registers
...
- [ ] Verify: split via stairs, regroup, branch/dungeon/plane split. _(playtest —
...
- [x] `Log.add(text, kind, who?)` with a host-side `audience` resolver:
 ...
 gray-paper / keys), the Warden rising + Amulet blink-steal, a party member
 ...
 target; the thief's rug to the victim; the Amulet of Yendor-reclaim to the recipient.
 ...
 *between* acts (timers, async flows, `draw`) sees the last-acted floor —
...
- **Netcode (UPDATED): co-op is now peer-authoritative deterministic lockstep**, not
 ...
 per-client side effects (relic-load) stay solo-gated. See the
...
5. Split into a branch/dungeon/plane; return; confirm persistence + no context bleed.
